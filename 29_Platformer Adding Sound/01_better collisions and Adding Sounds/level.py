        ...
        for row_index, row in enumerate(layout):
            for col_index, value in enumerate(row):
                ...
                # empty tile in Tiled is represented -1,
                    ...
                    if layer == 'coins':               
                        if value == '0':
                            tiles = gold = import_folder('../graphics/coins/gold')
                            sprite = AnimatedTile(x+16, y+16, tiles, 0.1250, 5)
                        elif value == '1':
                            tiles = silver = import_folder('../graphics/coins/silver')                      
                            sprite = AnimatedTile(x+16, y+16, tiles, 0.1050, 1)
        ...
        for sprite in self.terrain_sprites.sprites():
            if sprite.rect.colliderect(player.collision_rect):
                if player.direction.x < 0:
                    player.collision_rect.left = sprite.rect.right # +3
                    player.on_left = True
                    self.current_x = player.rect.left
                elif player.direction.x > 0:
                    player.collision_rect.right = sprite.rect.left # -3
    ...
    def check_coin_collisions(self):
        ...
        if collided_coins:
            ...
            for coin in collided_coins:
                self.change_coins(coin.coin_val)
