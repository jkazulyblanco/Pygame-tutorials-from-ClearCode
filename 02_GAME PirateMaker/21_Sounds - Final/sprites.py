...
class Shell(Generic):
    def __init__(self, orientation, assets, pos, group, pearl_surf, damage_sprites):
        ...
        if orientation == 'right':
            # flip all surfaces inside animation frames
...
class Player(Generic):
    def __init__(self, pos, assets, group, collision_sprites, jump_sound):
        ...
        self.gravity = 4
        self.on_floor = False
        self.pressed = False # update, do once
    ...
    def get_status(self):
        if self.direction.y < 0:
            self.status = 'jump'
        elif self.direction.y > 1: # 1 because 0 is for fall
            self.status = 'fall'
        else:
    ...
    def input(self):
        ...
        if keys[pygame.K_SPACE] and self.on_floor:
            self.pressed = True # update, do once
        else:
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and self.on_floor:
                    self.direction.y =- 2
                    self.jump_sound.play()
        ...
        for sprite in self.collision_sprites:
            if sprite.rect.colliderect(self.hitbox):
                if direction == 'horizontal':
                    ...
                    self.direction.y = 0 # reset to stop the gravity effect
