...
class Menu:
    ...
    def create_data(self):
        ...
        for key, value in EDITOR_DATA.items():
            if value['menu']:
                if not value['menu'] in self.menu_surf:
                    self.menu_surf[value['menu']] = [(key, load(value['menu_surf']))]
                else:
                    self.menu_surf[value['menu']].append((key, load(value['menu_surf'])))
        # print(self.menu_surf) # show the funcionality
    ...
    def create_buttons(self):
        ...
        Button(self.tile_button_rect, self.buttons, self.menu_surf['terrain'])
        Button(self.coin_button_rect, self.buttons, self.menu_surf['coin'])
        Button(self.enemy_button_rect, self.buttons, self.menu_surf['enemy'])
        Button(self.palm_button_rect, self.buttons, self.menu_surf['palm fg'], self.menu_surf['palm bg'])

    def click(self, mouse_pos, mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1]: # middle mouse click
                    # check if sprites has alternative item
                    if sprite.items['alt']:
